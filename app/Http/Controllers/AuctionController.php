...
class AuctionController extends Controller
{
    ...
    public function getAuctionById($auction_id)
    {
        try {
    ...
    public function updateAuction(Request $request)
    {
        try {
            ...
            // Linked teams update
            $teams = Team::where('auction_id', $auction_id)->get();
            foreach ($teams as $team) {
                $team->player_allow = (int) $auction->player_perteam;

                // Already bought players
                $playerBuy = (int)  $team->player_buy;

                // Update remaining players
                $team->player_remember = (int) $team->player_allow - $playerBuy;

                // Update reserve balance
                $team->reserve_balance = (int) $team->player_remember * $auction->minimum_bid;
    ...
    public function deleteAuction($auction_id)
    {
        try {
            ...
            Player::where('auction_id', $auction_id)->delete();
            Team::where('auction_id', $auction_id)->delete();
            $auction->delete();
